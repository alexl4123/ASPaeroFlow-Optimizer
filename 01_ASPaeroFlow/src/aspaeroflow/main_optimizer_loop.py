        ...
        if self._explainability_context is None:
            if self._controller_enabled is True:
                ...
                for event_key in events.keys():
                    if self._control_ctrl_socket == event_key:
                        ...
                        if command == "PAUSE":
                            ...
                            return optimization_dto, ("<LOAD>", command[6:])
                ...
                if paused:
                    ...
                    optimization_dto["paused"] = paused
                    return optimization_dto, "continue"
...
current_time -> {int(time_bucket_updated)}
    ...
    def run(self) -> None:  
        ...
        optimization_dto = self.setup_before_optimization()
               
        while np.any(optimization_dto["capacity_overload_mask"], where=True):

            optimization_dto, command = self.optimization_step(optimization_dto)
            ...
            optimization_dto = self.evaluate_solution(optimization_dto)

        return self.post_processing(optimization_dto)
